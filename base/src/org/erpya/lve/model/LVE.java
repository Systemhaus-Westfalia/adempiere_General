		...
		if(timing == TIMING_BEFORE_PREPARE) {
			...
			if (po.get_TableName().equals(MInvoice.Table_Name)) {
				...
				if(!invoice.isReversal()) {
					...
					/*
					 * if (new Query(invoice.getCtx(), MWHWithholding.Table_Name,
					 * "C_Invoice_ID = ? AND IsManual = 'N'",
					 * invoice.get_TrxName()).setParameters(invoice.getC_Invoice_ID()).match()) { //
					 * Get Document No int docNo = Integer.parseInt(invoice.getDocumentNo()); //
					 * Format Date String format = "yyyyMM"; SimpleDateFormat sdf = new
					 * SimpleDateFormat(format); String prefix =
					 * sdf.format(invoice.getDateInvoiced().getTime()); if(prefix == null) prefix =
					 * ""; // Set New Document No invoice.setDocumentNo(prefix + String.format("%1$"
					 * + 8 + "s", docNo).replace(" ", "0")); }
					 */
					//	Create Allocation
	...
	public String modelChange(PO po, int type) throws Exception {
		if(type == TYPE_BEFORE_NEW 
			...
			if (po.get_TableName().equals(MInvoice.Table_Name)) {
				 ...
				 * MWHWithholding withholding = (MWHWithholding) po; MInvoiceLine invoiceLine =
				 * new Query(withholding.getCtx(), MInvoiceLine.Table_Name,
				 * "C_InvoiceLine_ID = ?", withholding.get_TrxName())
				 * .setParameters(withholding.getC_InvoiceLine_ID()) .first(); if
				 * (invoiceLine!=null && invoiceLine.get_ID()>0) {
				 * invoiceLine.set_ValueOfColumn("InvoiceToAllocate_ID",
				 * withholding.getSourceInvoice_ID()); invoiceLine.save(); } }
				 */
